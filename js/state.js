(function () {
  ...
  const ADMIN_USER_IDS = Array.isArray(window.CANVAS_ADMIN_USER_IDS)
    ? window.CANVAS_ADMIN_USER_IDS.map((id) => String(id || "").trim()).filter(Boolean)
    : [];
  ...
  function isCurrentUserAdmin() {
    return isAdminUserId(getStoredUserId());
  }
  ...
  function normalizeCreationRecord(raw, idx) {
    if (!raw || typeof raw !== "object") return null;

    const normalized = {
      id: raw.id || raw.i || `legacy_${idx}_${makeId()}`,
      title: String(raw.title || raw.t || "Creacion").trim().slice(0, 60),
      author: String(raw.author || raw.a || getStoredUserName()),
      likes: Number.isFinite(raw.likes) ? raw.likes : Number(raw.l) || 0,
      boosts: Number.isFinite(raw.boosts) ? raw.boosts : Number(raw.b) || 0,
      comments: Array.isArray(raw.comments) ? raw.comments : (Array.isArray(raw.c) ? raw.c : []),
      remixOf: raw.remixOf || raw.r || null,
      createdAt: raw.createdAt || raw.ts || nowMs()
    };

    if (Array.isArray(raw.grid) && raw.grid.length) {
      normalized.grid = raw.grid;
      return normalized;
    }

    if (raw.g && typeof raw.g === "object") {
      normalized.grid = decodeGrid(raw.g);
      return normalized;
    }
  ...
  function unpackState(raw) {
    return {
      ...
      creations: Array.isArray(raw.cr)
        ? raw.cr.map((entry, idx) => normalizeCreationRecord(unpackCreation(entry), idx)).filter(Boolean)
        : [],
      openCanvas: {
  ...
  function parseState(raw) {
    ...
    try {
      ...
      state.creations = state.creations
        .map((entry, idx) => normalizeCreationRecord(entry, idx))
        .filter(Boolean);
  ...
  function stateScore(source) {
    if (!source) return 0;
    const creations = Array.isArray(source.creations) ? source.creations.length : 0;
    const events = Array.isArray(source.events) ? source.events.length : 0;
    const openPixels = Object.keys(source.openCanvas?.pixels || {}).length;
    return creations * 10 + openPixels * 2 + events;
  }

  function normalizeUsernameFromUser(user) {
    const raw = String(
    ...
    const base = raw.replace(/[^a-z0-9_]/g, "_").replace(/_+/g, "_").replace(/^_+|_+$/g, "");
    if (base.length >= 3) return base.slice(0, 24);
    return `user_${String(user?.id || "anon").slice(0, 8)}`;
  }

  async function ensureRemoteProfile(client, user) {
    if (!client || !user?.id) return { settings: {} };
    ...
    const username = normalizeUsernameFromUser(user);
    ...
    // Reintento con username único por si existe colisión del alias.
    const uniqueUsername = `user_${String(user.id).slice(0, 8)}`;
    await client
      .from("profiles")
      .insert({
        id: user.id,
  ...
  async function bootstrapRemoteState() {
    if (remoteBootstrapStarted) return;
    ...
    try {
      ...
      const ensured = await ensureRemoteProfile(client, user);
      const settings = ensured.settings && typeof ensured.settings === "object" ? ensured.settings : {};
      remoteSettingsCache = settings;

      const remotePacked = settings?.app_state;
      if (remotePacked && typeof remotePacked === "object") {
        const candidate = parseState(JSON.stringify(remotePacked));
        if (stateScore(candidate) >= stateScore(state)) {
          state = candidate;
          localStorage.setItem(STORAGE_KEY, JSON.stringify(packState(state)));
        }
      }
    } catch {
      // No rompemos la app si falla la sincronización remota.
    } finally {
      remoteBootstrapDone = true;
    }
  }
  ...
  function scheduleRemoteSave() {
    ...
    remoteSaveTimer = setTimeout(async () => {
      try {
        ...
        if (payload === remoteLastPayload) return;

        const baseSettings = remoteSettingsCache && typeof remoteSettingsCache === "object"
          ? remoteSettingsCache
          : {};
        const nextSettings = { ...baseSettings, app_state: packed };

        await ensureRemoteProfile(client, user);

        const { error } = await client
          .from("profiles")
          .upsert({
            id: user.id,
            username: normalizeUsernameFromUser(user),
            settings: nextSettings
          }, { onConflict: "id" });

        if (error) return;
        remoteSettingsCache = nextSettings;
        remoteLastPayload = payload;
      } catch {
        // Silencioso: se mantiene modo local si falla remoto.
      }
    }, 500);
  }
  ...
  function getCreationById(id) {
    ...
    for (const candidate of candidates) {
      ...
      const found = state.creations.find((item) => String(item.id).trim() === normalized);
  ...
  function buyItem(type) {
    const user = state.user;
    const allowedTypes = new Set(["pixels_50", "speed_upgrade", "charge_now", "max_charge_plus", "editor_grid_plus"]);
    if (!allowedTypes.has(type)) return { ok: false, reason: "invalid_item" };
  ...
  function setDisplayName(nextName) {
    const safeName = String(nextName || "").trim().slice(0, 32);
    if (!safeName) return { ok: false, reason: "empty" };
    state.user.name = safeName;
    save();
