(function () {
  ...
  function buy(itemId) {
    ...
    if (!result.ok) {
      ...
      if (result.reason === "invalid_item") alert("Este item ya no esta disponible en la tienda.");
    } else {
  ...
  function render() {
    ...
    Object.entries(grouped).forEach(([sectionName, sectionItems]) => {
      ...
      section.innerHTML = `<h2 class="shop-section-title">${sectionName}</h2>`;

      const sectionGrid = document.createElement("div");
      sectionGrid.className = "shop-grid-section";
